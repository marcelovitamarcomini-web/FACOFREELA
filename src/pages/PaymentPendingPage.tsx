...
export function PaymentPendingPage() {
  ...
  return (
    <div className="container max-w-4xl space-y-8 py-10 sm:space-y-10 sm:py-12 lg:py-14">
      <section className="rounded-[36px] border border-amber-200 bg-[linear-gradient(180deg,#ffffff_0%,#fff9ef_100%)] p-6 shadow-[0_18px_50px_rgba(15,23,42,0.05)] sm:p-8">
        <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-amber-700">
          Pagamento pendente
        </p>
        <h1 className="mt-3 text-[2rem] font-semibold leading-[1.03] tracking-[-0.045em] text-slate-950 sm:text-[2.7rem]">
          O checkout ainda não foi concluído.
        </h1>
        <p className="mt-4 text-sm leading-7 text-slate-600 sm:text-base">
          Sua conta freelancer pode continuar em estado pendente até a confirmação do pagamento.
          Acompanhe tudo pela área de assinatura.
        </p>
      </section>
        ...
        {status ? (
          ...
          <div className="rounded-[28px] border border-slate-200 bg-white px-5 py-5">
            ...
            <p className="mt-3 text-sm text-slate-600">
              Status do checkout: <span className="font-semibold text-amber-700">{checkout?.status ?? '--'}</span>
            </p>
          </div>
        )}
